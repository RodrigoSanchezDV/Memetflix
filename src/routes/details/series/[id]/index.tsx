...
import type { Genre } from "~/models/models_series_details";

export const useRouteLoader = routeLoader$(async (requestEvent) => {
    const id = requestEvent.params.id;
    const response = await api_service.details_series({ id });
    return response;
});
        ...
        <>
            <div class="flex justify-center  text-white pt-5 font-bold">
                <div class="w-3/4 text-center">
                    <h2 class="text-red-500 text-3xl mb-5">{response.value.name}</h2>
{/*                     <div class="flex justify-start border border-gray-800 mt-[4px] mb-[4px] p-[4px]">
                        <h2 class="mr-[5px]">Original title</h2>
                        <h2 class="">"{response.value.name}"</h2>
                    </div> */}
                    <div class="flex justify-start border border-gray-800 mt-[4px] mb-[4px] p-[4px]">
                        <h2 class="mr-[5px]">Last air date</h2>
                        <h2 class="text-orange-300">{response.value.last_air_date}</h2>
                    </div>
                    <div class="flex justify-start border border-gray-800 mt-[4px] mb-[4px] p-[4px]">
                        <h2 class="mr-[5px]">Number of seasons</h2>
                        <h2 class="text-orange-300">{response.value.number_of_seasons}</h2>
                    </div>
                    <div class="flex justify-start border border-gray-800 mt-[4px] mb-[4px] p-[4px]">
                        <h2 class="mr-[5px]">Number of episodes</h2>
                        <h2 class="text-orange-300">{response.value.number_of_episodes}</h2>
                    </div>
                    <div class="flex justify-start border border-gray-800 mt-[4px] mb-[4px] p-[4px]">
                        <h2 class="mr-[5px]">Status</h2>
                        <h2 class="text-orange-300">{response.value.status}</h2>
                    </div>
